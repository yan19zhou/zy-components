/*
 * @Author: cqg
 * @Date: 2021-05-19 09:44:13
 * @LastEditTime: 2021-05-19 10:06:57
 * @Description: 饿了么弹窗拖拽（参照于民间文档：https://blog.csdn.net/sinat_21902709/article/details/86545444）
 * @LastEditors: cqg
 */
export default {
  install(Vue) {
			...
			// 向顶部移动
              if (t < 0) {
                ...
                dragDom.style.top = `${t + styT}px`;
				  ...
				  // 向底部移动
                if (scrollHeight - dragDom.offsetTop > 10) {
                  dragDom.style.left = `${l + styL}px`;
                  dragDom.style.top = `${t + styT}px`;
                }
              }
			  // 已到达顶部
				...
				//  向底部移动
              if (t > 0 && scrollHeight - dragDom.offsetTop > 10) {
                // 移动当前元素
                dragDom.style.left = `${l + styL}px`;
                dragDom.style.top = `${t + styT}px`;
              }
