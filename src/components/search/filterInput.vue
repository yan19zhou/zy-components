<template>
  <div :class="`inputItem ${showAll ? 'small' : ''}`">
    ...
    <template v-else-if="inputType === 'cascader'">
      ...
      ></el-cascader> -->
      <Destination ref="provinceCity" :minLevel="min" @clearLevel="min = ''" v-model="filterValue" :name="name"/>
    </template>
    <template v-else-if="inputType === 'tree'">
      <DivisionTree ref="divisionTree" v-model="filterValue" :name="name"/>
    </template>
  </div>
</template>
...
import Destination from "./area.vue";
import DivisionTree from "./divisionTree.vue";
const MIN_NUMBER = 0;
...
export default {
  ...
  props: {
    ...
    inputType: {
      ...
      default: "input",
      validator(value) {
        return ["input", "dict", "radio", "checkbox", "select", "date", "datetime", "daterange", "datetimerange", "numberRange", "year", "month", "cascader", "tree"].includes(value);
      },
    },
